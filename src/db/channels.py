...
class ChannelOperationRepository:
    ...
    async def list_top(self, *, fork: int, limit: int) -> List[Tuple[bytes, int, int]]:
        """(channel_id, op_count, last_height) for the most active channels on the fork's chain."""
    ...
    async def count(self, channel_id: bytes, *, fork: int) -> int:
        chain = chain_block_ids_cte(fork=fork)
        statement = (
            ...
            .join(Block, ChannelOperation.block_id == Block.id)
            .join(chain, Block.id == chain.c.id)
            .where(ChannelOperation.channel_id == channel_id)
        )
        with self.client.session() as session:
            return session.exec(statement).one()

    async def get_operations(
        self, channel_id: bytes, *, fork: int, newest_first: bool, offset: int = 0, limit: int
